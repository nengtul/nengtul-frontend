...
function NoticeView(){
    ...
    useEffect(() => {
        if(MY_TOKEN!==null){
            getData<Post>(`${NOTICES_LIST_URL}/${noticeId}`,MY_TOKEN)
            .then((post:Post)=>{
                setPost(post);
    ...
    const [editedData, setEditedData] = useState<UpdateData>({ //수정할때 받아올 원래 내용
        title: '',
        content: '',
    });
    useEffect(() => {
            ...
            {isEditing?(
                ...
                <>
                    <Notice>
                        공지사항
                        <div className='btns'>
                            <button className='delete-btn btn' onClick={onModify}>취소</button >
                            <button className='modify-btn btn'  onClick={onSave}>완료</button >
                        </div>
                    </Notice>
                    <NoticeTitleDate>
                        <NoticeTitle>
